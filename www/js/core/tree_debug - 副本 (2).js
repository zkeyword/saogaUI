								...
								if( data ){
									...
									for(var i = 0; i<data.length; i++){
										if( Number(data[i].pId) === pId ){
											...
											if( isCheckBox ){
												checkHtml = '<span class="l-tree-check l-tree-checkbox l-tree-checkbox-'+ level +'" data-level="'+ level +'"></span>';
											}
					...
					eventFn: function(){
						...
						target
							...
							.on('dblclick', '.l-tree-node', function(e){
								var that      = $(e.currentTarget),
									data      = itemData(that),
									switchBtn = that.prev('.l-tree-switch'),
								...
								var that    = $(e.currentTarget),
									...
									checkFn = function(obj, level){
													...
													if( !isChecked ){
														...
														parentCheck.addClass('l-tree-checkbox-checked-part');

														checkNum       = parents.find('.l-tree-check[data-level="'+ level +'"].l-tree-checkbox-checked').length;
														partCheckedNum = parents.find('.l-tree-check[data-level="'+ level +'"].l-tree-checkbox-checked-part').length;
														
														console.log(botherNum, bother)
